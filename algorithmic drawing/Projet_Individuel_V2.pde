...
void setup() {
  ...
  for (int i = 0; i < columns; i++) {
    ...
    } //on a donc toutes les tailles et couelurs des carrés présents dans chaque cellule
  }
}
...
void draw() {
  ...
  float widthCell = width / float(columns); //chaque cellule contenant les carrés a la même taille : utilisation de width pour un code adaptable facilement
...
//interactivité : lorsqu'on clique, le carré sélectionné change de couleur et de taille
void mousePressed() {
  float widthCell = width / float(columns);
  float heightCell = height / float(rows);
  
  int i = int(mouseX / widthCell); //pour avoir les coordonnées du carré sélectionné
  int j = int(mouseY / heightCell);
